...
[`pir-sdk-wasm`](https://github.com/Bitcoin-PIR/Bitcoin-PIR/tree/main/crates/sdk/wasm)
...
| `WasmOramClient` | One-server Direct ORAM client (AMD SEV-SNP). The free backend today; see [Payments](/docs/sdk/payments). |
| `WasmArcCredentialRequest`, `WasmArcCredential` | Credits: the blinded ARC credential request and the credential it finalizes into. |
...
[five invariants](/docs/privacy/invariants).
...
A server that requires credits refuses metered frames until the
connection is funded. Every client has `enableCredits(...)`: call it after
`upgradeToSecureChannel` with a credit provider, and the client presents
credits (`REQ_CREDIT_PRESENT`) whenever a connection's balance runs short.
`presentCredits(...)` is the one-shot primitive underneath. Who charges,
the price, and a complete wallet are on the [Payments](/docs/sdk/payments)
page.
