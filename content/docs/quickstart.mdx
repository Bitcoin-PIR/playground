...
<Callout kind="note" title="Free while the servers have room">
The reference servers serve DPF free on a best-effort lane: paid lookups
go first, and when the free lane is full a server answers
`free capacity busy` instead. A [credit provider](/docs/sdk/payments)
added after the sealed channel (step 5) pays for priority then;
HarmonyPIR hints and OnionPIR on `pir1` always need one.
</Callout>
