...
<Callout kind="warn" title="pir1 requires credits">
Since 2026-09-15 the hint / DPF server 0 (`pir1`) charges
[credits](/docs/sdk/payments) for every metered frame. Without a credit
provider the query below stops at pir1's first metered frame with
`credits required`. Add one after the sealed channel (step 5), or start
with the free Direct ORAM backend — the [Payments](/docs/sdk/payments) page
has both.
</Callout>
...
inside `crates/sdk/wasm/`, and `npm link` it. See
[`crates/sdk/wasm/README.md`](https://github.com/Bitcoin-PIR/Bitcoin-PIR/blob/main/crates/sdk/wasm/README.md)
...
for (let i = 0; i < sync.resultCount; i++) {
  ...
  // SOUNDNESS: `merkleVerified === false` means a per-bucket proof
  // failed and the server's response cannot be trusted. Skip the slot
  // (and surface the failure to the user); never display the entries.
  if (!result.merkleVerified) {
    console.warn(`${i}: Merkle FAILED — refusing to display untrusted UTXOs`);
    continue;
  }
  console.log(
